...
export type MapperReportAudience = "CISO" | "SOC leader" | "Technical architecture";

const disclaimer = "Independent community planning tool. Not a Microsoft assessment, quote, licensing determination, private detection implementation description, or guaranteed savings statement.";

function linesFor(audience: MapperReportAudience, analysis: MapperAnalysis): string[] {
  const sources = analysis.sources.slice(0, 12);
  const lead = audience === "CISO"
  ...
  return [
    ...
    ...sources.flatMap((source) => [
      `${source.normalizedSourceName} | ${source.sourceFamily} | ${source.observedGBPerDay.toFixed(2)} GB/day | ${source.securityValue} | ${source.detectionReadiness}`,
...
export async function exportCloudSecurityReport(
  ...
  const doc = new jsPDF();
  const title = audience === "CISO" ? "Cloud Security Value Assessment" : `Cloud Security Value Mapper - ${audience} Report`;
  doc.setFontSize(18);
  doc.text(title, 20, 24);
  doc.setFontSize(9);
  doc.text(customerLabel ? `Report label: ${customerLabel}` : "Report label: not provided", 20, 32);
